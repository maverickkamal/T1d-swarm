...
runner = Runner(
    agent=forecast_verifier_agent,
    app_name=APP_NAME,
    session_service=session_service
)
print(f"Runner configured for agent: {forecast_verifier_agent.name}")

# --- Define Agent Interaction Logic (for testing) ---
async def test_verifier_agent_with_mock_state(
    ...
    risk_forecast_data: dict   # mock forecast data
):
    """
    Sets up session state with mock data, runs the verification forecaster,
    ...
    trigger_message_text = "Generate verification forecast based on current state."
    ...
    stored_forecast_objec = updated_session.state.get(forecast_verifier_agent.output_key)

    print(f"\n  --- Content of session.state['{forecast_verifier_agent.output_key}']: ---")
...
async def main_test_runner():
    print("--- Starting verification Forecaster Agent Test Scenarios ---")
    ...
    # await test_verifier_agent_with_mock_state(session_id_stable, mock_cgm_stable, mock_context_stable)
    ...
    # await test_verifier_agent_with_mock_state(session_id_hyper, mock_cgm_hyper, mock_context_hyper)
    ...
    # await test_verifier_agent_with_mock_state(session_id_hypo, mock_cgm_hypo, mock_context_hypo)
    ...
    # await test_verifier_agent_with_mock_state(session_id_erratic, mock_cgm_erratic, mock_context_erratic)
    ...
    # await test_verifier_agent_with_mock_state(session_id_missing, mock_cgm_missing, mock_context_missing)
    ...
    # await test_verifier_agent_with_mock_state(session_id_stress, mock_cgm_stress, mock_context_stress)
    ...
    # await test_verifier_agent_with_mock_state(session_id_illness, mock_cgm_illness, mock_context_illness)
    ...
    # await test_verifier_agent_with_mock_state(session_id_complex_meal, mock_cgm_complex_meal, mock_context_complex_meal)
    ...
    await test_verifier_agent_with_mock_state(session_id_conflicting, mock_cgm_data_conflicting_high, mock_context_event_conflicting_high, mock_forecast_conflict)
    ...
    await test_verifier_agent_with_mock_state(session_id_old_context, mock_cgm_data_old_context, mock_context_event_old_context, mock_forecast_old)
    ...
    await test_verifier_agent_with_mock_state(session_id_multi_event, mock_cgm_data_multi_event, mock_context_event_multi_event_exercise, mock_forecast_exercise)
    ...
    await test_verifier_agent_with_mock_state(session_id_unexplained_rise, mock_cgm_data_unexplained_rise, mock_context_event_unexplained_rise, mock_forecast_no_context)
    ...
    await test_verifier_agent_with_mock_state(session_id_very_high, mock_cgm_data_very_high, mock_context_event_very_high, mock_forecast_unwell)
