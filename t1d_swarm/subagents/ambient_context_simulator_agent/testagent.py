...
APP_NAME = "ambient_context_agent"
...
ambient_context_agent = LlmAgent(
    model=MODEL_NAME,
...
runner = Runner(
    agent=ambient_context_agent,
    app_name=APP_NAME,
    session_service=session_service
)
print(f"Runner configured for agent: {ambient_context_agent.name}")

# --- Define Agent Interaction Logic (for testing) ---
async def test_ambient_agent_with_scenario_state(
    ...
    # 3. Run the agent
    final_context_json_str = "No forecast generated."
    async for event in runner.run_async(user_id=USER_ID, session_id=session_id_for_test, new_message=user_content):
        if event.is_final_response() and event.content and event.content.parts:
            final_context_json_str = event.content.parts[0].text # This will be the JSON string
    ...
    print(final_context_json_str) # The raw JSON string from the agent

    stored_forecast_objec = updated_session.state.get(ambient_context_agent.output_key)

    print(f"\n  --- Content of session.state['{ambient_context_agent.output_key}']: ---")
...
async def main_test_runner():
    print("--- Starting Ambient Context Agent Test Scenarios ---")
    ...
    await test_ambient_agent_with_scenario_state(session_id_very_high, mock_scenario_random)
